...
<div id="app" class="app-container">
        ...
        <div class="ios-card">
            ...
            <el-table :data="keys" style="width: 100%" empty-text="暂无数据">
                ...
                <el-table-column label="API Key" min-width="180">
                    <template #default="scope">
                        <span @click="copyKey(scope.row.key)" style="font-family: monospace; color: var(--text-sub); cursor: pointer; transition: color 0.3s;" title="点击一键复制" onmouseover="this.style.color='var(--theme-pink)'" onmouseout="this.style.color='var(--text-sub)'">
                            {{ maskKey(scope.row.key) }}
                        </span>
                    </template>
                </el-table-column>
    ...
    <div v-show="activeTab === 'models'">
        ...
        <div class="ios-card">
            ...
            <p style="font-size: 13px; color: var(--text-sub); margin-bottom: 16px;">
                点击下方按钮获取硅基流动官网提供的所有模型，并在列表中直接打勾选择。
            </p>
...
<script>
    ...
    const app = createApp({
        setup() {
            ...
            const appToken = ref('813a8d31a5a246f6be2e7294284269c1');
            const appMessage = ref('你好，这是一条通过本地 Web 代理发送的联动测试指令。');
            const appResult = ref('');
            ...
            const fetchRawModels = async () => {
                ...
                try {
                    const res = await fetch('/api/raw_models');
                    const data = await res.json();
    ...
    try:
        ...
        for _ in range(10):
            if not check_port(PORT):
                local_ip = get_local_ip()
                print("\n✅ 代理启动成功！")
                print(f"   平板管理面板: http://127.0.0.1:{PORT}")
